...
| **0.32.0** | **Speed & phone** | **A37** (server-side time window — the timeline loads a date range and grows as it scrolls, the map gets a thin geo endpoint, and the statistics become SQL aggregates instead of a client-side reduce) · **A38** (the mobile layout pass ch. 5.7 has been owed since the beginning). Together because they are the same complaint from the same place: the app on a phone, on a real database. A37 is the last release that may change the API shape freely — after it the demo dataset and the upgrade promise start to hold things still. | M–L + S–M |
| **0.33.0** | **Demo mode** (R1a) | A seeded, entirely fictional dataset behind one flag: a plausible life with trips, places across several continents, sightings, concerts, journal entries, weather, achievements and a handful of freely licensed images. **This is the release that unblocks everything public.** | M–L |
| **0.34.0** | **Hardening & operations** (R1c/d/f) | `AUTH_MODE=dev` unstartable in a production-shaped environment · no secrets in logs · pinned base images · Dependabot · `SECURITY.md` · versioned ghcr images and a genuine `docker compose up` · **the upgrade path from an older database tested end to end** · backup and restore documented, media folder included. | M–L |
| **0.35.0** | **Project surface** (R1b/e/g) | README with screenshots and a short GIF · the “why not X” comparison table from ch. 1.1 · `CONTRIBUTING.md` · issue templates · questions to Discussions · “what this project deliberately does not do” · the donation link (note 63). | S–M |
| **0.36.0** | **Freeze & fresh-install pass** | No new features. Walk the stranger's path from an empty machine, fix what that turns up, tidy the docs, verify every `.env.example` key is real and every documented command works. Bug fixes only from here. | S–M |
...
**Risk to watch** *(rewritten 2026-07-21)*. The long runway the earlier draft worried
about is nearly walked: only **0.32.0** now stands between here and the demo mode, and
it earned its place — the app is measurably too slow on the author's own data, and
slowness is the first thing a stranger would meet too. What remains worth watching is
the opposite failure: A37 touches the API shape, so it must land **before** the demo
dataset and the upgrade promise freeze it, not after. Should the runway need shortening
after all, the order of retreat is the **second stage of 0.25.0** (Immich as an event
source) and then **A38** (the mobile pass can be split, its findings are independent of
each other). A37 and the demo mode are not negotiable: one decides whether the app is
usable at size, the other whether anyone gets to look before installing.
