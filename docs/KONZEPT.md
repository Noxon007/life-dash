...
| **F13** | **Selectable background maps** *(note 51)* | S–M | The map has one tile style per theme today (F9 switches light/dark). New: a **layer switcher** on every map (timeline, collection, world, tracks) with a small bundled set — OSM standard, light and dark (Carto), OpenTopoMap, satellite (Esri World Imagery) — **plus a freely configurable tile URL template** in the settings, so no provider is hardwired (A27). Attribution is per layer and is never optional. The choice is stored per device and survives the theme switch; the light/dark automatism becomes the default rather than a rule. | Satellite imagery is what people actually want on a holiday map, topographic layers are what they want on a hike — and the custom template means the project never has to pick a favourite provider. |
| **F14** | **“On this day”** *(note 53, from the market analysis)* | S | Every journal application has memory resurfacing; Life-Dash does not. A block on the start view: events from this calendar day 1, 5, 10, 20 years ago — with the photo (later, P2.1), the weather and the place. A pure layer-4 derivation over `date_start`, respecting `date_precision` (only `exact`/`day`/`month` qualify — a `year` memory has no day to hit). Optionally a daily notification via the PWA. | The largest emotional payoff per line of code in the whole roadmap: the database stops being an archive and starts talking back. |
