...
| **0.34.1** *(or on `main`)* | **Badges** | **F19** (note 99): the tier ladder stops ending at platinum and keeps counting against a generated next mark; the thresholds that bulk import made trivial are lifted once. **Before the demo dataset on purpose** — 0.35 renders the achievements tab, and a fictional life that is instantly platinum in everything misrepresents the feature to exactly the audience the demo exists for. No schema consequence, so per note 89 it needs no version of its own and may simply ship on `main` ahead of 0.35. | S–M |
