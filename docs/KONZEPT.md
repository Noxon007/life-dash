...
| **A5** | **Dekaden-Aggregation & Besuchs-Verdichtung** *(vorher P3.2)* | M | Heat-/Dichte-Darstellung auf Jahrzehnt-/Jahres-Ebene statt langer Kartenlisten; wiederholte Besuche desselben Orts gruppieren („59× Besuch: X"), Alltagsorte (Zuhause/Arbeit) optional zusammenfassen. **Dazu (Frage 14):** den harten Karten-Deckel (aktuell max. 300 Marker pro Zeitraum, DOM-Schutz) durch **Marker-Clustering** ersetzen — es werden immer *alle* Punkte repräsentiert, bei Dichte gebündelt statt abgeschnitten. | Übersicht trotz zehntausender Timeline-Events — aktuell die größte Nutzungs-Bremse. |
...
| **A8** | **Export-Rückmeldung** *(Frage 13)* | S | Der Export-Button lädt still eine Datei herunter. Neu: Button während des Ladens deaktivieren (wie beim Import), nach Erfolg Toast mit Inhaltsangabe („Export erstellt: N Events, M Fragmente, … → Dateiname"), bei Fehler Fehler-Toast. | Nutzer weiß sicher, ob das Backup geklappt hat. |
| **A9** | **Logging & Beobachtbarkeit** *(Frage 15)* | M | Zentrale Logging-Konfiguration statt vereinzelter Logger: ein `lifedash.*`-Logger-Baum, `LOG_LEVEL` per Env (Compose), einheitliches Format mit Zeitstempel. Geloggt werden: Requests (uvicorn access log an), KI-Aufrufe (Modell, Dauer, Fehler), Geocoding-/Wetter-Aufrufe, Import-Etappen (Datei, Anzahl, Dauer), Admin-Aktionen, Fehler mit Traceback. Alles nach stdout → `docker logs`; Compose bekommt Log-Rotation (`max-size`/`max-file`). Optional später: Log-Ansicht im Admin-Panel. | Fehlersuche im Homelab ohne Rätselraten — heute ist das Backend fast stumm. |
| **A10** | **Ortsnamen konsequent auf Deutsch/Latein** *(Frage 16)* | S | Geocoding sendet zwar `Accept-Language: de`, aber ohne deutschen OSM-Namen liefert Nominatim den **lokalen** Namen (z. B. griechische Schrift). Fix: Fallback-Kette `Accept-Language: de,en` (englisch = lateinische Umschrift), zusätzlich `namedetails=1` mit Namenswahl `name:de → name:en → name` — für Geocoding, Reverse-Geocoding und „Ortsnamen auflösen". Dazu Admin-Aktion, bestehende nicht-lateinische Ortsnamen erneut aufzulösen (nutzergestartete Datenverbesserung, Kap. 3.1). | Lesbare Ortsnamen weltweit. |
...
| **A13** | **Uhrzeiten anzeigen & nutzen** *(Frage 22)* | S | Exakte Zeiten sind längst da (Timeline-Import speichert Start-/Endzeit als lokale Wanduhrzeit, `date_precision=exact`), werden aber nirgends angezeigt — `exact` rendert wie `day`. Entscheidung zum Umgang: `exact` = **Datum + Uhrzeit** anzeigen (z. B. „12.07.2026, 14:30–16:05", Endzeit nur bei gleichem Tag), `day` = nur Datum (bleibt Standard für manuelle Eingaben); Bearbeiten-Dialog bekommt optionale Uhrzeit-Felder; die KI darf `exact` setzen, wenn der Text eine Uhrzeit nennt. Zeitzone: bewusst Wanduhrzeit „wie erlebt", keine UTC-Umrechnung (dokumentiert). | Tagesabläufe (Timeline-Import!) werden lesbar; kein Datenmodell-Umbau nötig. |

**Empfohlene Reihenfolge in A** (A1–A3 sind erledigt): **A5**
(Übersichts-Problem) zuerst; die kleinen **A8/A10/A13** eignen sich als
schnelle Zwischenschritte; dann **A9** (Logs helfen bei allem Weiteren) →
A12 (baut auf A10 auf) → A4 → A11 → A6 → A7.
