...
| **Standort, Wetter & Länder (v0.14.0, 2026-07-19)** | **F2** 📍-Knopf bei KI-Analyse (Koordinaten ins Fragment, Ortsvorschlag nur wenn der Text keinen Ort nennt) und manueller Eingabe (Adresse ins Ortsfeld, `/api/ingest/reverse-location`). **F3** Wetterbedingung aus Stundendaten (dominantes Tagwetter 8–20 Uhr), `temp_min_c`/`temp_max_c`/`precipitation_mm` als neue Metriken; Bestand bleibt unangetastet. **F4** Land aus addressdetails → `Location.country` + `country`-Entity mit Links auf alle Events des Orts (idempotent), greift bei Ortsauflösung, Vorwärts-Geocoding und Standort-Erfassung; rückwirkend über die Auflösungs-Läufe. |
