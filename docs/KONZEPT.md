...
| 154 | **Map controls** — the display row mixes layers, a drawing and a mode, and “merge points” secretly also decides whether the map shows everything. | Analysed with three worked-out designs (note 154). The choice is the author’s: option B changes a control people already know. **Finding (b) is out of it and built** (note 159): the paths switch being silently dead above month zoom was a violation of an existing rule (A40), not a design question. (a), (c) and (d) are still the open ones. |
