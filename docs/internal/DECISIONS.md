...
171. ⛔ **`Image "arrow" could not be loaded` on the vector basemap.** *(Moot since note 206: the vector basemap is gone, and this remedy with it. Kept because the reasoning — say it once, in the place where the choice is made — outlives the feature.)* A third-party style asks for sprite icons its own sprite sheet does not contain. MapLibre reports this **per icon and per tile**, so panning produces hundreds of identical lines that push every real message out of the console. The fix is the one MapLibre documents: answer `styleimagemissing` with a transparent 1×1 image. Said **once** in the console, and — because whoever runs this instance never opens a console — as a line under the style URL in the settings, where the choice is made.
...
**Round of 2026-08-09, second pass (note 203), on `main`, no version bump.**
The second of the four steps agreed before publication: the demo dataset. It is
R1a, the part the roadmap calls “the one that unblocks all the others”.

203. ✅ **A fictional life, thirty-two years long, built rather than derived.** R1a. The old seed put seven German sentences through `ingest_fragment`; for seven sentences that was right, because it showed the path fragment → proposal → life database. For thirty years it is the wrong instrument, and the reason has nothing to do with speed.

    **The result would otherwise hang on the AI provider.** The same flag would produce one corpus with the mock provider and another with Gemini, every screenshot would be a one-off, and a recognition error would look like a defect in the demo. What is written now is the *result* of the pipeline, which is the only promise a shop window should make. The three layers stay visible anyway: a handful of unprocessed fragments and four unconfirmed proposals ride along, because a corpus in which everything is confirmed hides half the application.

    **Reproducible, not random.** The biography is written down (`demo/life.py`): residences, trips, concerts, milestones. What is rolled is only what is arbitrary in a real corpus too — which Tuesday somebody went running — and that from a fixed seed. Twice built is twice the same corpus, or no screenshot is repeatable and no measurement against it can be run twice.

    **The weather is invented and does not touch the network.** Eleven thousand days of real archive would mean half an hour of waiting on first start, a corpus that cannot come into being without a connection, and figures that differ between two builds. Values are plausible for latitude and season and derive from a hash over coordinate and date — a hash rather than `random`, because it is stable across Python versions. **The one genuinely computed value is day length**: “longest day” is a ranking in this app, and an invented solar cycle would turn it into a statement about the random number generator. Its output has exactly the key set of `weather.fetch_weather`, asserted in both directions — a missing key is an empty tile, an invented one goes nowhere silently.

    **And it carries the revision marker.** Without it the weather run would consider the entire demo corpus unprocessed and fire eleven thousand Open-Meteo requests on the first click — the endless-refetch trap in new clothing, this time built in by the *writer* rather than the fetcher. Writing that guard is what exposed the round's one real defect: the builder's cut-off for “too recent to have weather” was a hard-coded 6, counted from *yesterday*, while `enrichment._too_recent` counts `ERA5_LAG_DAYS` from *today*. Two spellings of one rule, two days apart, and the visible consequence was exactly two days of entries that the run would have picked up again. It asks the rule now instead of copying it.

    **The blank stretch is deliberate.** Seventy-five days in 2020 have no residence and no entries at all — the move abroad was delayed and nobody recorded anything. Without it the gap report is empty, and **a view that has nothing to say in the demo looks like a broken one**. It is defined once (`life.BLANK`) and read by three generators; typed three times, the next shift would close it in two places and leave it open in the third, and the result would not be an error but a quiet half-measure.

    **Two more things the corpus is deliberately not.** It is not platinum in everything — 5 of 18 achievements at the top tier, six at bronze — for the reason the roadmap wrote down when it pulled F19 forward: a life that is instantly maxed misrepresents two features to exactly the audience the demo exists for. And nothing is dated later than yesterday: the application treats the future as a special case everywhere (no weather, no scoring), and a shop window should not show the special case as the norm.

    **The pictures are generated, not shipped.** Photo strips, the lightbox, pictures on a *day* (F18) and the photo panel are four views that stay empty without a single image, and an empty view in a shop window reads as “cannot do that”. A bag of real photos in the repository would be a licence and a size problem for everyone who clones it, and fetching freely licensed ones would mean the demo build needs a connection after all. Four hundred and sixty generated gradients are a few kilobytes of code, identical on every build, and **they declare themselves as placeholders** — an abstract field with the place and date written on it claims nothing, whereas an image that looks like a photograph and is not would be the one dishonesty a *life* database does not get forgiven. They go through `media.store`, the same path an upload takes; a separate write path would be a second kind of uploaded picture, and the one no test knows about.

    **What the demo deliberately does not show is the map's photo layer.** Since note 139 that layer is made of events with source `immich`, and it lives on thumbnails a foreign server delivers. Without an Immich instance those would be thousands of dead links — a broken image at every point is worse than no point — so it stays what it is: a connector feature. Both other kinds of attachment do appear, at an event and at a day, because the photo panel reads `captured_at` falling back to the event's date and filling only one half would produce a summary that looks complete.

    Guards: twenty cases in `test_r1a_demo.py`, the corpus built **once per module** in its own database — five seconds times ten would be half this project's test time for something that does not change between two assertions. Its own engine also on the PostgreSQL run, because the `db` fixture empties the shared one before every test and a module-wide corpus on it would be gone after the first foreign test, with every assertion afterwards passing against an empty database. What is dialect-dependent about it — the foreign keys, which the builder writes past with core inserts — is asserted by a case that **only runs on PostgreSQL**, since on SQLite it would be green in any event.

    Two of those guards are about the pictures and neither asserts that they exist: one checks that **every row has its file and its thumbnail on disk**, because a row without a file is a broken image on every card in the timeline and it only shows in a browser, where nobody is looking; the other checks that most of them sit on *trips*, because evenly spread photographs would be a statement about the generator rather than about a life, and the photo years would come out as a straight line. The test module redirects `media_dir` at a throwaway directory — without that the suite would write four hundred JPEGs into the folder that in operation holds the only data the JSON export cannot carry.

    One assertion had to be rewritten after the counter-check: “nothing lies after *now*” is a promise against the wall clock. A visit written at 7 a.m. of the current day is past by nine and future by six, so the same broken loop was green or red depending on when the suite ran. The builder's rule is “the everyday ends yesterday”, and the loop that broke it counted up *before* it checked — pinned now with a fixed boundary instead of the clock.

**Round of 2026-08-09, third pass (note 204), on `main`, no version bump.**
The third of the four steps: measure against the demo corpus, then settle the
findings notes 199 and 201 deliberately left as measurements. Four of the five
turn out to be non-problems, which is the point of measuring. The one real
finding was not on the list.

204. ✅ **Measured against the shipped corpus — and the first thing it measured was the measuring stand.** `tools/_measure_api.py` exists so that this project rebuilds against numbers rather than feelings (notes 179/182). Since R1a it broke: dev mode now seeds a fictional life at startup, so the tool piled its twenty thousand events *on top of* one and measured a size that exists nowhere. It aborted on the uniqueness of residence days, which is the lucky ending — a silent result would have cost more.

    **And the load case was not one.** With `DEMO=1` the same run now measures the corpus the application ships, and the comparison is the finding: the demo is **four times slower on the expensive endpoints than the “load case”, with less than half the events**. The reason is one line of the fixture: the synthetic corpus writes *three* weather values per residence day, the application writes *seventeen*. A load case that underestimates the costliest dimension six-fold does not measure, it reassures. Both corpora stay — one is the stress, the other is what a stranger will actually run.

    **The two suspects from note 199 are not the problem, and the question is closed rather than open.** `_place_ranking` costs 27 ms at 3,673 places and **199 ms at 28,673** — the “one place per photo at twenty thousand photos” case its own comments warn about, built on purpose to measure it. `_farthest_from_home`: 33 ms and 181 ms. Capping either would change *which answer comes out*, in exchange for two hundred milliseconds. That is a bad trade, and it is now written down with a number instead of an intuition.

    **Two more from note 201, likewise settled.** `_day_weather_candidates` costs 42 ms per pass; over three hundred batches that is thirteen seconds inside a run that spends hours waiting on Open-Meteo. And the unbounded `IN (…)` lists do not bite: a full backup round trip of 288,428 rows completes (SQLite 61 s, PostgreSQL 110 s) including a 126,361-element list, because psycopg2 binds parameters client-side and the server's 65,535 limit never applies. **That is a property of the driver, not of the code** — with psycopg3 or asyncpg the same list is a hard error, so it is recorded here rather than called safe.

    **The one that was real: the export read every account's rows.** `metrics` and `event_entity_links` carry no `user_id`; their owner is the event they hang on. The export loaded them *whole* and picked its own out in Python. The file was always correct — what was wrong is that it was **read** for. Measured: a second account in the system cost a user **a third of their export time** (4.8 s → 6.4 s) for data of which not one row reaches their file; with the ownership in the join it is +7 %. The guard counts **rows loaded**, not rows written: a test on the file's contents stays green if the filter wanders back into Python tomorrow, which is exactly the regression to catch. It counts ORM objects rather than matching the SQL text, because whether the author writes `JOIN` or `IN (SELECT …)` is their business — that no foreign row is loaded is the promise.

    **And the finding nobody had on the list: the statistics tab spends 94 % of its time on weather.** `compute_overview` measured 2,326 ms, of which `_weather_stats` was 2,285 ms, of which `weather_day.day_values` was 1.3 s — **152,854 rows into Python, from which four numbers and a bar chart came out**. Nothing about that had been decided badly; the row count simply grew by an order of magnitude when F20 gave every residence day seventeen weather values, and the question above it never changed. `year_totals` now groups in SQL and returns **one row per year, thirty-three instead of a hundred thousand**. End to end: 2,326 → 1,710 ms.

    **Two stages, and the second is the whole point.** First one row per (day, key) — that is the rule, the cautious minimum of note 119 — then the sum over days. Aggregating the raw rows directly would let an imported day with thirty visits count thirty times, which is A31 all over again. The staging lives in `_per_day_key`, read by **both** `day_values` and `year_totals`: written out twice, the two would drift the moment somebody replaces the minimum, and the “rainy days” tile would then contradict the day row you open to check it.

    **The near-miss is worth more than the change.** It is obvious to narrow the query to `rain_mm` and `sunshine_h` — only two are needed. That would have made “days with weather” count *days with a rain value*, quietly dropping any day carrying only a temperature. The tile would have kept looking right and merely been smaller. The key set therefore belongs to the caller, where `_WX_KEYS` already lives; and the measurement says the narrowing would have bought nothing anyway — 13 keys cost 449 ms, 2 keys 159 ms, but the day count alone over 13 keys costs 290 ms, so splitting saves zero.

    **What is left, stated rather than hidden:** the remaining 1.7 s is the *event* side — `weather_values` loads every event's weather into Python and `_extreme_tops` ranks it for the record tiles. That is the same shape of problem one table over, and it is deliberately untouched here: it decides *which row wins* in twelve rankings, and that is a change to make on its own, with its own guards.
...
**Round of 2026-08-09, second set (notes 206 onwards), on `main`, no version
bump.** Asked for as: strike the tested upgrade path, strike the vector map,
build the hardening and the project surface, and close every open note that is
not a post-1.0 feature.

206. ✅ **The vector basemap is withdrawn — A48 in full, together with note 171 and note 149's console remedy.** Not a defect report: *“it causes too much trouble”*, and the record agrees with that summary rather than softening it.

    **What it cost, in the order the costs appeared.** An LCP above 4.9 s when the map switched to week or month, with no cause visible in our own code and none found in two rounds of looking (still open when this was decided). A third-party style asking for sprite icons its sprite sheet does not contain, answered in note 171 with a transparent 1×1 pixel and a line in the settings. Two more libraries from a CDN — **MapLibre and the Leaflet bridge, half of the four that note 200 lists as loading with no integrity hash and no CSP**. A dependency on WebGL, which is absent on older devices and in hardened environments. And a failure mode where all of that looks the same on screen: grey.

    **What it bought.** One basemap option that only worked at all if the operator first pasted a style URL out of Immich's admin settings. There is exactly one operator, and the three tile maps plus *“🔧 Custom map”* answer the same question.

    **This is a removal, not a deprecation.** The style URL and its attribution lived in `localStorage` on the device — nothing in any database refers to them, so there is no migration and nothing to keep. A stored basemap choice of `vector` resolves through `basemapSetting()`, which already falls back to `auto` for any key `BASEMAPS` does not know: the case was covered before it existed. Gone with it: `vectorSupported`, `makeBaseLayer`'s second drawing path, `vectorWatchMissingImages`, `vectorMissingNote`, the settings block, twelve catalogue keys and `tools/check-vector-basemap.js`.

    **The guard was deleted rather than rewritten, and that is worth stating.** It asserted that a vector map never fails silently — a rule about a feature that no longer exists. Keeping it green by weakening it is how a test suite comes to assert the past; the thirty-eight remaining guards run unchanged.
...
208. ✅ **The hardening, R1(d): six of note 200's ten open findings, and one of them was wrong on the first attempt.** Not a report — the list has been sitting in note 200 since 2026-08-07, written down rather than built because hardening is a decision and the decisions were the author's. They were made on 2026-08-09: *close everything that is not a post-1.0 feature.*

    **The dev mode does not start any more where it must not.** `AUTH_MODE=dev` means no login at all; `get_dev_user()` answers every request with the account that owns everything. The old code said so in a comment and logged a warning — and a warning in the log is this project's documented way of hiding a defect. It is now an abort, and the check does not ask *“is this instance reachable?”* (a process cannot know) but *“does it look like it is meant to be?”*, at the two places the accident actually happens: `PUBLIC_BASE_URL` not pointing at this machine, and a configured OIDC provider next to `AUTH_MODE=dev` — the half-finished switch-over, where the operator has already written down an intention that contradicts the setting. `DEV_AUTH_ALLOW_PUBLIC=true` is the deliberate exception, because a public **demo** instance is a real use case, and it re-announces itself on every start: an exception nobody sees again is not an exception, it is the setting. The default `SESSION_SECRET` became an abort in the same move — there is no case in which a real login should be signed with a value published in `.env.example`.

    **The CSP, and why it needed note 207 first.** With the libraries in the house, `script-src` can say `'self'` instead of naming a foreign origin. The page is one file with two inline blocks, so the easy answer was `'unsafe-inline'` — which would have surrendered exactly the class the directive exists for (an injected `<script>` in a place name, a journal entry, an Immich album title). It carries the SHA-256 of the two blocks instead, computed **at startup from the file this process serves**, not written into the source: a hash in the source is a rule in two places, and it would be wrong the first time anyone edits a line.

    **And that is where this note earned its keep.** The first version hashed the bytes on disk. The HTML specification requires every CRLF to become LF *before* tokenising, so the browser hashes something else — on a Windows working copy (CRLF) the hashes were wrong, in the Linux container (LF) they were right. A defect that appears exactly where the work happens, disappears exactly where it is verified, and shows itself as a page that simply does nothing. It was caught by computing the same two hashes in a real HTML parser (jsdom) and finding they disagreed. The test that guards it is therefore not “the hash is what the function returns” — that asserts nothing — but two properties: **the same file with CRLF and with LF must hash identically**, and one fixture is checked against a value produced by jsdom, in another language.

    A hash in `script-src` makes the browser drop **every** inline handler. The single `onclick=""` in the file — the reload link in the connection banner — moved to an event listener; left alone it would have been a dead link that looks alive. `esc()` learned `'`, which until now held only because all 363 attributes in the file happen to use double quotes: a guarantee that depends on a typing habit is not one.

    **Secrets in the log: a filter, not an audit.** An audit answers “does today's code log a secret?”; it does not answer for the `log.info("%s", url)` someone leaves in while debugging. And this instance *displays* its log in the admin panel (A17), so a key in the ring buffer is a key on a web page. Two paths, because there are two kinds of secret: the configured ones (whole-value replacement), and the ones this process cannot know — above all the **per-user Immich key**, which lives in the database, and which a config-only filter would have missed entirely. That one is caught by pattern (`key=`, `api_key=`, `Bearer …`).

    **The filter hangs on the handlers, and that is not taste.** Python applies only the *originating* logger's filters when a record propagates upward — `Logger.callHandlers` calls handlers, never ancestors' filters. A filter on the `lifedash` logger would have looked like it protected the whole tree and done nothing for `lifedash.admin`, `lifedash.geocode`, `lifedash.immich`: a guard that misses precisely the cases it exists for. Verified by moving it there and watching the test go red.

    **The raw admin view gave out what `_settings_view` refuses by name.** `users.password_hash` and the Immich key — the latter inside `users.settings`, so a check on column names would never have found it. One list, read from both directions: reading redacts, **writing is refused**. Redacting alone is half an answer — being unable to read a password hash while being able to *set* one takes over every account on the instance with a hash you chose yourself. And `raw_text` got its upper bound; it protects not the database, which takes anything, but the AI request behind it, which costs tokens per call and had no ceiling.

    Guards: 23 cases in `test_anm208_hardening.py`. Run against the broken states — the newline normalisation (three red), the filter on the logger instead of the handlers (one red). The counter-directions are deliberate and are the reason the set is trustworthy: dev mode on localhost still starts, a real login with its own secret still starts, `not-needed` as a placeholder key is **not** redacted (a filter that blacks out everything makes the log useless and gets switched off), `/docs` keeps working, and an admin can still edit an email address.

    **What stays open from note 200, and it is now three rather than ten:** the bearer-token audience, the login lockout's unbounded table, and revocable sessions. They are the next note — they touch the auth path and want their own round rather than a paragraph at the end of this one.

209. ✅ **The last three of note 200: revocable sessions, a bounded lockout table, and an audience the bearer path actually checks.** They were kept out of note 208 on purpose — all three touch the sign-in path, and a paragraph at the end of another round is how a change to authentication gets made without being thought about.

    **Sessions could not be revoked, and the one action a person takes did nothing.** A stolen cookie was valid for thirty days, and changing the password — the thing a human does when they suspect someone else has access — left it valid. The obvious answer, a session table, answers the wrong question: it tells you *which* sessions exist, and nobody here asks that. The question asked is *does this one still count?*, and one timestamp per user answers it. The cookie now carries its issue time; anything older than `users.sessions_valid_from` is dead. No write per request, no table that grows, and no second place a deleted account has to be removed from.

    The cost is stated rather than discovered later: **individual devices cannot be signed out, only all of them.** For a system with one operator and no device list, “sign out everywhere” is the action that actually exists. A password change performs it and immediately re-issues the current session, because a change that logs you out of the window you made it in reads as a failure.

    Three details decide whether this works at all. The cutoff is set **one second into the future**: signing in and immediately changing the password falls in the same second, and `iat >= cutoff` would have spared exactly the cookie being revoked. A cookie **without** an issue time — every cookie written before this change — counts as revoked, because the alternative is that the old sessions, the very ones somebody is revoking, are the only survivors. And `NULL` means “never revoked”, so nobody is signed out by the update itself.

    **The lockout table had no upper bound**, and one entry per attempted address means memory handed out by strangers. It is capped at 5,000, expired entries first and then the oldest window. That ordering is the whole design: pruning **after** writing the new entry means an attacker who is actively guessing has the *newest* window and is the last thing evicted — a cap that let a flood of invented addresses wash away a running lock would have traded the brake for memory. The honest other half is a test of its own: a lock nobody is pushing any more **can** be evicted. It expires in fifteen minutes regardless, and getting it back requires guessing again, which re-creates it.

    **The bearer path accepted any token the issuer had signed** (`verify_aud=False`) — the entire purpose of the `aud` claim, switched off. With one application at the identity provider that is invisible; with two, a token a user handed to the *other* application is a valid login here. It now checks against the client ID, with `OIDC_AUDIENCE` for providers that mint access tokens for a resource rather than a client — without that escape hatch the only remaining option would have been to switch the check off again. The rejection reason goes to the **log, not the response**: an attacker should not learn what their token failed on, and an operator whose integration suddenly stops working cannot be left guessing.

    Guards: 13 cases in `test_anm209_sessions.py`, plus the whole suite against PostgreSQL (`pwsh tools/pg-test.ps1`) because this adds a column. Two of the thirteen were written after the first version failed them, and both were the code being wrong rather than the test: pruning ran *before* the insert (so the table sat one over its cap, and the freshly written entry was not protected), and the first flood test asserted a property the design does not have. The counter-directions: a fresh cookie stays valid, a cookie issued after a revocation works again, a timezone-aware cutoff from PostgreSQL still compares, and an account with no revocation is untouched.

210. ✅ **The operational half of R1(d): a container that is not root, a base image that is a build rather than a moving tag, and a mechanism that keeps both current.** The remaining hardening from note 200, plus the two things the gate asked for that were never about code.

    **Root was the finding; the bind mount is why it survived four versions.** `./data` and `./media` come from the host with the host's ownership, and an image that starts directly as an unprivileged user cannot write into a directory that already exists — which shows up on *upgrade*, as an instance that no longer comes up. The entry point therefore does what mature self-hosted images do: start as root, hand over exactly the two writable directories, drop privileges, then start the application. Not `chown -R /`: the code stays root-owned and read-only to the application, which is the point of the exercise.

    Three details are deliberate. The user id is **fixed at 10001** rather than assigned by the system, because it becomes visible on the host — whoever backs the files up has to know whom they belong to, and a new id on every image rebuild would make that unanswerable. Setting `user:` in Compose skips the handover entirely, because an explicit statement by the operator should win over convenience. And if neither `setpriv` nor `su` exists, the container **refuses to start** rather than continuing as root: a fallback to “then root it is” would remove the hardening and tell nobody, which is the exact shape this whole round exists to eliminate.

    **A digest without Dependabot is a tin can.** `python:3.13-slim` points somewhere new most weeks — convenient for security updates, useless as a statement about what was built. It is pinned by digest now, with the tag beside it as a comment so the line stays readable. That trade only works with something that lifts it again, so the two shipped together; the Actions are in there as well, and they are third-party code that runs with write access to the container registry.

    **`--forwarded-allow-ips "*"` was made configurable and left at `*`, and that is a decision rather than a repair.** Trusting forwarded headers from anyone lets whoever reaches the app port directly claim any client address — which is what the login lockout counts against and what the logs record. But the proxy has no fixed address in a Docker network, and a wrong value here produces an application that believes it is running unencrypted: a silent failure, and this project rates silent worse than open. So the default stays, `FORWARDED_ALLOW_IPS` names the proxy for anyone who knows it, and `DEPLOY.md` says that not publishing the port at all settles the question better than either.

    **`SECURITY.md` exists to be read before an evening is spent.** Private reporting, one maintainer, no bounty — and, more usefully, a list of what is known and deliberate: the dev mode's missing authentication, the open first registration, revocation that is all-or-nothing, the per-email lockout, tiles fetched from the operator's chosen server. Next to it, what is *not* deliberate, which is the part worth someone's time: cross-account reads, stored text that executes, secrets in a response or a log, a path that reaches the host. Those are the four classes this project has repeatedly found in itself.
...
**Round of 2026-08-09, fifth pass (note 206), on `main`, no version bump.** The
same feedback thread, one layer up. Note 205 made the server link every day;
the user then asked why one day still had fourteen photo entries and no strip,
and whether the two Immich runs could become one. Both answers were yes, and
one of them cost a rule.

206. ✅ **Two runs answering one question, and a browser making note 205's mistake all over again.**

    **The display bug first, because it is the reported one.** The photo strip was pinned to the event list twice: `loadDayMedia(page)` derives the query's date range from the loaded *events*, and — the real defect — the strip was only inserted for days appearing in `g.items`. So a day with pictures and no entry had no group, no day heading and no strip, no matter what the server had linked. **This is note 205's `FROM` clause one storey up:** a set over days enumerated from events. F20 had already solved the same problem for residence days by injecting their rows *before* grouping; photo days now do the same, with a marker row that is removed again once the strip is in place. It stays out when a day, a city or a search is selected — those are selections over *entries*, and a day carrying only pictures is not one.

    **Then the merge, which was the user's proposal:** *“can we make the Immich jobs into one? all photos get checked; with coordinates it becomes a located event, and the pictures get attached to the day; without coordinates just the pictures — even where there is no entry yet.”* That is almost exactly what the code already did, in two halves that never met. `photo_points` walked the library by year with a preview; `immich` attached pictures to days. **Both asked Immich the same question** — *what lies in this window?* — and each answered one half, so which day ended up with pictures depended on which run had gone last and how far. One grab per month now yields both.

    **What the merge cost, and it is not a detail: the preview is gone.** It was enforced in the UI (`check-p21-preview.js`) because only the browser knows what somebody *saw*, and it was the single brake in front of a run that writes confirmed life database — the moderation for this source has been gone since note 138. The user removed it with a reason stronger than the rule: *“in the doing I am not going to look at 8,000 proposals, and I am not going to do it month by month either.”* **A brake nobody presses is not a brake**; it only lengthens the path to the result. What replaces it is not nothing: every row carries the slot `immich:photo:<asset>`, so it is machine-findable, and `/api/photos/reset` takes them back with their tombstones. Undo instead of preview. The guard was rewritten rather than deleted — `check-immich-run.js` now watches the *undo* path, including that discarding photo **events** asks with the number while discarding **links** does not, because one is life database and the other a derivation.

    **The nightly plan keeps it**, also the user's call, reversed mid-discussion after first agreeing to drop it. It means a machine creates confirmed entries unattended. Written down here as a decision rather than an oversight.

    **What did NOT change, and the reason is worth more than the change would have been.** The plan was one event per day+place instead of one per photo, to stop fourteen “Photo in Bad Segeberg” rows. Reading the code first killed it: `compressVisits` already collapses machine entries of a day **by title**, so the timeline *shows* exactly one row per day+place. The visible noise the user objected to was already gone. Making it real data would have cost the map — its photo layer is one point per photo and its popup resolves the picture through the asset id in `external_id` — plus a reversal of note 139, a new cleanup run, and migrating 8,187 confirmed rows, for a timeline that looks identical. **I had argued for the change before checking, and the user had agreed to it on my argument.** Saying so was the useful part of that round.
